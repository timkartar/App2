...
public class HeavyLifter {
     ...
     */
    public void setResourceAsWallpaper(final Bitmap resourceId) {
        new Thread() {
            @Override
            public void run() {
                try {
                    manager.setBitmap(resourceId);
     ...
     */
    //private Bitmap getImage(Bitmap resourceId) {
        //Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(), resourceId, null);
       // Bitmap scaledBitmap = Bitmap.ScaledBitmap(bitmap, manager.getDesiredMinimumWidth(), manager.getDesiredMinimumHeight(), true);
        //bitmap.recycle();
        //bitmap = null;
        //return scaledBitmap;
    //}
